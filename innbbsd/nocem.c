...
$Id: nocem.c,v 1.1.1.1 2009-03-04 06:33:28 bmybbs Exp $
*/
...
char   *issuer, *type;
{
	...
	for (i = 0; i < NCMCOUNT; i++) {
		...
		if ((!strcmp(find->issuer, "*") || !strncmp(issuer, find->issuer,length)) 
...
verify_buffer(char *buf, char *passphrase)
{
	...
	char	Buf[1024];
	int     ans = NOPGP;
	run_pgp("/usr/bin/gpg --decrypt --batch >/dev/null",
		&pgpin, &pgpout);
	if (pgpin && pgpout) {
