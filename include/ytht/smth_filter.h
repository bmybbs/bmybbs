/* utf8 source code */

#ifndef BMYBBS_SMTH_FILTER
#define BMYBBS_SMTH_FILTER
#include "ytht/fileop.h"

/*
 * 您的文章可能含有按照国家有关规定不适宜在公共场合发表的内容
 * 本文须经过有关人士审查后发表
 * 如果您有疑问，请联系在线的站长审核该文章
 */

#define BAD_WORD_NOTICE "\xC4\xFA\xB5\xC4\xCE\xC4\xD5\xC2\xBF\xC9\xC4\xDC\xBA\xAC\xD3\xD0\xB0\xB4\xD5\xD5\xB9\xFA\xBC\xD2\xD3\xD0\xB9\xD8\xB9\xE6\xB6\xA8\xB2\xBB\xCA\xCA\xD2\xCB\xD4\xDA\xB9\xAB\xB9\xB2\xB3\xA1\xBA\xCF\xB7\xA2\xB1\xED\xB5\xC4\xC4\xDA\xC8\xDD\n" \
	"\xB1\xBE\xCE\xC4\xD0\xEB\xBE\xAD\xB9\xFD\xD3\xD0\xB9\xD8\xC8\xCB\xCA\xBF\xC9\xF3\xB2\xE9\xBA\xF3\xB7\xA2\xB1\xED\n" \
	"\xC8\xE7\xB9\xFB\xC4\xFA\xD3\xD0\xD2\xC9\xCE\xCA\xA3\xAC\xC7\xEB\xC1\xAA\xCF\xB5\xD4\xDA\xCF\xDF\xB5\xC4\xD5\xBE\xB3\xA4\xC9\xF3\xBA\xCB\xB8\xC3\xCE\xC4\xD5\xC2"

/*
 * 按照有关部门要求,本版文章必须经过有关人士审查后发表,请耐心等候!
 * 如果您有疑问，请联系在线的站长审核该文章
 */
#define DO1984_NOTICE  "\xB0\xB4\xD5\xD5\xD3\xD0\xB9\xD8\xB2\xBF\xC3\xC5\xD2\xAA\xC7\xF3,\xB1\xBE\xB0\xE6\xCE\xC4\xD5\xC2\xB1\xD8\xD0\xEB\xBE\xAD\xB9\xFD\xD3\xD0\xB9\xD8\xC8\xCB\xCA\xBF\xC9\xF3\xB2\xE9\xBA\xF3\xB7\xA2\xB1\xED,\xC7\xEB\xC4\xCD\xD0\xC4\xB5\xC8\xBA\xF2\x21!\n" \
	"\xC8\xE7\xB9\xFB\xC4\xFA\xD3\xD0\xD2\xC9\xCE\xCA\xA3\xAC\xC7\xEB\xC1\xAA\xCF\xB5\xD4\xDA\xCF\xDF\xB5\xC4\xD5\xBE\xB3\xA4\xC9\xF3\xBA\xCB\xB8\xC3\xCE\xC4\xD5\xC2"

enum ytht_smth_filter_option {
	YTHT_SMTH_FILTER_OPTION_SIMPLE = 0, /** 对于普通版面简单过滤 */
	YTHT_SMTH_FILTER_OPTION_NORMAL = 1, /** 对于zz相关的版面 */
	YTHT_SMTH_FILTER_OPTION_PLTCAL = 2, /** 特殊版面 */
};

enum ytht_smth_filter_result {
	YTHT_SMTH_FILTER_RESULT_SAFE = 0, /** 不做处理 */
	YTHT_SMTH_FILTER_RESULT_1984 = 1, /** 内部转载 */
	YTHT_SMTH_FILTER_RESULT_WARN = 2, /** 可能危险 */
};

int ytht_smth_reload_badwords(char *wordlistf, char *imgf);
int ytht_smth_filter_string(const char *string, struct mmapfile *badword_img);
int ytht_smth_filter_article(char *title, char *filename, struct mmapfile *badword_img);

#endif

