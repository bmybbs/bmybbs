...
static void ythtbbs_mybrd_sync_ext(const struct user_info *ptr_info, ythtbbs_mybrd_has_read_perm_ext func);
...
void ythtbbs_mybrd_load_ext(const struct user_info *ptr_info, struct goodboard *mybrd, ythtbbs_mybrd_has_read_perm_ext func) {
	...
	sethomefile_s(buf, sizeof(buf), ptr_info->userid, GOODBRD);
	if ((fp = fopen(buf, "r"))) {
		for (mybrd->num = 0; mybrd->num < GOOD_BRD_NUM;) {
			if (!fgets(buf, sizeof(buf), fp))
				break;
			...
			ytht_strsncpy(mybrd->ID[mybrd->num], ytht_strtrim(buf), sizeof(mybrd->ID[mybrd->num]));
			if (func(ptr_info, mybrd->ID[mybrd->num])) {
				mybrd->num = mybrd->num + 1;
			}
		}
...
int ythtbbs_mybrd_save_ext(const struct user_info *ptr_info, struct goodboard *mybrd, ythtbbs_mybrd_has_read_perm_ext func) {
	...
	ythtbbs_mybrd_sync_ext(ptr_info, func);
	return count;
}
...
static void ythtbbs_mybrd_sync_ext(const struct user_info *ptr_info, ythtbbs_mybrd_has_read_perm_ext func) {
	...
	memset(&local_mybrd, 0, sizeof(struct goodboard));
	ythtbbs_mybrd_load_ext(ptr_info, &local_mybrd, func);
	...
	bmy_subscription_sync(1 + ythtbbs_cache_UserIDHashTable_find_idx(ptr_info->userid), bnums, real_num);
