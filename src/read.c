...
long
get_num_records_excludeBottom(char *filename, int size) ; //·µ»Ø²»°üº¬ÖÃµ×µÄÌû×ÓÊý
...
//Èç¹ûdef_topline>=1, Èç¹ûÃ»ÓÐÕÒµ½µÄ»°, Ä¬ÈÏµÄµÚÒ»Ò³Ò³Âë
//Èç¹ûdef_topline==-1, Èç¹ûÃ»ÓÐÕÒµ½µÄ»°, ²»¸øÄ¬ÈÏÒ³, ¶øÊÇ·µ»Ø¿ÕÖ¸Õë
struct keeploc *
	...
	if (last_line == 0) {
		if (cmdmode == RMAIL) {
			...
			if (friendflag)
				...
				strcpy(desc, "»µÈË");
			sprintf(buf, "Ã»ÓÐÈÎºÎ%s (A)ÐÂÔö%s (Q)Àë¿ª£¿[Q] ", desc,
				desc);
			...
			if (!(IScurrBM) || !club_board(currboard)) {
				getdata(t_lines - 1, 0,
					"¿´°æÐÂ³ÉÁ¢ (P)·¢±íÎÄÕÂ (Q)Àë¿ª£¿[Q] ",
					genbuf, 4, DOECHO, YEA);
				if (genbuf[0] == 'p' || genbuf[0] == 'P')
					do_post();
			} else {
				getdata(t_lines - 1, 0,
					"¿´°æÐÂ³ÉÁ¢ (P)·¢±íÎÄÕÂ (N)ÉèÖÃ¾ãÀÖ²¿³ÉÔ± (Q)Àë¿ª£¿ [Q] ",
					genbuf, 4, DOECHO, YEA);
				if (genbuf[0] == 'p' || genbuf[0] == 'P')
					do_post();
...
char *direct;
{
	...
	static const char *SR_BMitems[] =
	    { "²»¼õÎÄÕÂÊýÉ¾³ý", "±£Áô", "ÎÄÕª", "·ÅÈë¾«»ªÇø", "·ÅÈëÔÝ´æµµ",
		"ÉèÎª²»¿É»Ø¸´", "ºÏ¼¯" ,"¼õÎÄÕÂÊýÉ¾³ý"//add heji by bjgyt
	};
	...
	getdata(t_lines-2, 0, "Ö´ÐÐ: (0) È¡Ïû  (1) ÏàÍ¬Ö÷Ìâ  (2) ÏàÍ¬×÷Õß  [0]: ", ch, 3, DOECHO, YEA);	//mint
	dotype =  atoi(ch);
	...
	move(t_lines - 3, 0);
	clrtoeol();
	sprintf(buf, "È·¶¨ÒªÖ´ÐÐ%s[%s]Âð", subBMitems[dotype-1], SR_BMitems[BMch - 1]);
	...
	if(BMch == 7){			//add by mintbaggio 040321 for heji
		...
		// ¸üÐÂwwwµ¼¶ÁÏÂµÄÁ´½Ó by IronBlood 20130805
...
/*ÏÈÕÒµÚÒ»Æª, ÔÙÕÒµÚÒ»ÆªÐÂµÄ, Èç¹ûÕÒµ½ÁË, ¾Í¶ÁÖ®, ·ñÔòÖ±½Ó·µ»Ø*/
int
...
struct fileheader *ptitle;
{
	...
	if (passonly == 0 || passonly == 4) {
		if (readfirst)
			...
			clrtoeol();
			prints
			    ("[1;44;31m[%8s] [33mÏÂÒ»·â <Space>,<Enter>,¡ý,n©¦ÉÏÒ»·â ¡ü,U,l                          [m",
	...
	while (!istest) {
		...
		if (passonly == 0 || passonly == 4) {
			...
			if (ch != KEY_UP && ch != KEY_DOWN
				...
				clrtoeol();
				prints
				    ("\033[1;44;31m[%8s] \033[33m»ØÐÅ R ©¦ ½áÊø Q,¡û ©¦ÏÂÒ»·â ¡ý,n,Enter©¦ÉÏÒ»·â ¡ü,l©¦ ^R »Ø¸ø×÷Õß   \033[m",
				     auser ? "ÏàÍ¬×÷Õß" : "Ö÷ÌâÔÄ¶Á");
				ch = egetch();
			}
			switch (ch) {
...
//´Ó locmem Î»ÖÃÆð£¬Ïòoffset·½Ïò£¬ËÑË÷threadÏàÍ¬µÄfileheader£¬
//mode == 0 ËÑË÷µÚÒ»¸ö¼´ÍË³ö£¬·ñÔòËÑË÷µ½×îºóÒ»¸ö¸ººÉÌõ¼þµÄ
{
	...
	match = 0;
	if (digestmode == 0 || digestmode == 3 || uinfo.mode == BACKNUMBER) {	//Õâ¼¸ÖÖ.DIRÊÇÅÅÐòµÄ
		sorted = 1;
	} else {
		sorted = 0;
	}
	MMAP_TRY {
		...
		if (mode == 0) {	//°¤×ÅËÑ£¬ËÑµ½¾ÍÍ£
			...
			while (1) {
				if (offset > 0) {
					...
					if (--now < 1)
						break;
					pFh--;
					if (sorted && pFh->filetime < thread)	//ÓÉÓÚ.DIRÅÅÐò£¬ËÑµ½ÕâÀï¾Í¿ÉÒÔÍ£ÁË
						break;
				}
			...
			if (sorted && offset == -1) {	//ÓÐÍû¼ÓËÙµÄ²¿·Ö
				start = Search_Bin(mf.ptr, thread, 0, now - 1);	//Á½·Ö·¨ÕÒµ½ËÑË÷Æðµã
