...
struct proc_entry my_proc[5] = {
	// CPU1温度
	{SENSOR_P "temp1", "CPU1\xCE\xC2\xB6\xC8", "%*s %*s %20s"},
	// CPU2温度
	{SENSOR_P "temp2", "CPU2\xCE\xC2\xB6\xC8", "%*s %*s %20s"},
	// FAN1转速
	{SENSOR_P "fan1", "FAN1\xD7\xAA\xCB\xD9", "%*s %20s"},
	// FAN2转速
	{SENSOR_P "fan2", "FAN2\xD7\xAA\xCB\xD9", "%*s %20s"},
	{0, 0, 0}
};
...
/* load control for BBS */
#ifdef LOAD_LIMIT
	{
		...
		load = cpu_load[0];
		if (big5)
			// \033[1;36mBBS 最近 \033[33m(1,5,15)\033[36m 分鐘的平均負荷分別為\033[33m %.2f, %.2f, %.2f \033[36m(目前上限 = %d).\033[0m\n\r\n\r
			prints("\033[1;36mBBS \xB3\xCC\xAA\xF1 \033[33m(1,5,15)\033[36m \xA4\xC0\xC4\xC1\xAA\xBA\xA5\xAD\xA7\xA1\xAD\x74\xB2\xFC\xA4\xC0\xA7\x4F\xAC\xB0\033[33m %.2f, %.2f, %.2f \033[36m(\xA5\xD8\xAB\x65\xA4\x57\xAD\xAD = %d).\033[0m\n\r\n\r",
			...
			prints("\033[1;36mBBS \xD7\xEE\xBD\xFC \033[33m(1,5,15)\033[36m \xB7\xD6\xD6\xD3\xB5\xC4\xC6\xBD\xBE\xF9\xB8\xBA\xBA\xC9\xB7\xD6\xB1\xF0\xCE\xAA\033[33m %.2f, %.2f, %.2f \033[36m(\xC4\xBF\xC7\xB0\xC9\xCF\xCF\xDE = %d).\033[0m\n\r",
					cpu_load[0], cpu_load[1], cpu_load[2], max_load);
	...
	if (checkbansite(hid)) {
		if (big5)
			// 本站目前不歡迎來自 %s 訪問!\r\n
			prints("\xA5\xBB\xAF\xB8\xA5\xD8\xAB\x65\xA4\xA3\xC5\x77\xAA\xEF\xA8\xD3\xA6\xDB %s \xB3\x58\xB0\xDD!\r\n", hid);
		else
			// 本站目前不欢迎来自 %s 访问!\r\n
			prints("\xB1\xBE\xD5\xBE\xC4\xBF\xC7\xB0\xB2\xBB\xBB\xB6\xD3\xAD\xC0\xB4\xD7\xD4 %s \xB7\xC3\xCE\xCA!\r\n", hid);
		shutdown(csock, 2);
		close(csock);
		exit(-1);
	}
			...
			if (addrcheck[i].x > 100 && addrcheck[i].n > 7) {
				if (j == 0 && fd >= 0)
					if (fork() == 0) {
						...
						char *s = "\xB6\xD4\xB2\xBB\xC6\xF0, \xC1\xAC\xBD\xD3\xBD\xAB\xB7\xE2\xB1\xD5\x35\xB7\xD6\xD6\xD3\xA1\xA3\xC7\xEB\xB2\xBB\xD2\xAA\xB2\xBB\xB6\xCF\xC1\xAC\xBD\xD3\xB3\xE5\xBB\xF7\xB1\xBE\xD5\xBE\n";
