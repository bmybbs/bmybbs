...
show_cake(char *filename, int num)
{
	...
	i = 0;
	prints("\033[1;28H\033[35mA PIECE OF CAKE\033[2;29HĞ¡ ²Ë Ò» µú"
	       "\033[0m\n\n\n");
	while (NULL != fgets(line, 200, fp)) {
		...
		if (line[0] == '#' && line[1] != '#') {
			...
			if (i == 0) {
				sprintf(line, " ÇëÊäÈëÊı×Ö'%d': ", num0 % 10);
				getdata(20, 1, line, str, 29, DOECHO, YEA);
				if (atoi(str) != num0 % 10) {
					prints
					    ("\033[22;1H??..ºÃÏó²»´ó¶Ô°É, Äú»¹ÓĞÒ»´Î»ú»á...");
					getdata(20, 1, line, str, 29, DOECHO,
						YEA);
					if (atoi(str) != num0 % 10) {
						prints
						    ("°¡? ¾¹È»»¹²»¶Ô? ÕæµÄÊÇÄãÃ´, ÎÒºÃÉËĞÄ...");
						pressreturn();
						Q_Goodbye();
					}
				}
...
post_article(struct fileheader *sfh)
{
	...
	modify_user_mode(POSTING);
	if (! ((strcmp(currboard, "welcome") == 0) && (strcmp(currentuser.userid, "guest") == 0)))
	{
		if (!haspostperm(currboard))
		{
			move(3, 0);
