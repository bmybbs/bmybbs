...
void showstuff(char *buf) {
	...
	static char numlogins[10], numposts[10], rgtday[35], lasttime[35],
	    thistime[35], lastlogout[35], stay[24], alltime[32], ccperf[20],
	...
	// %ld小时%ldx分钟
	snprintf(alltime, sizeof alltime, "%ld\xD0\xA1\xCA\xB1%ld\xB7\xD6\xD6\xD3", (long int) (currentuser.stay / 3600),
		(long int) ((currentuser.stay / 60) % 60));
	snprintf(rgtday, sizeof rgtday, "%24.24s", ctime(&currentuser.firstlogin));
	snprintf(lasttime, sizeof lasttime, "%24.24s", ctime(&currentuser.lastlogin));
	snprintf(lastlogout, sizeof lastlogout, "%24.24s", ctime(&currentuser.lastlogout));
	snprintf(thistime, sizeof thistime, "%24.24s", ctime(&now));
	snprintf(stay, sizeof stay, "%ld", ((time(0) - login_start_time) / 60));
