	...
	for (i = 0; i < USHM_SIZE; i++) {
		...
		if ((*fptr) (farg, uentp)) {
			if (show == 0)
				...
				j++;
				// (%d) 目前在干嘛: %s, 来自: %s \n
				prints("(%d) \xC4\xBF\xC7\xB0\xD4\xDA\xB8\xC9\xC2\xEF: %s, \xC0\xB4\xD7\xD4: %s \n", j, ModeType(uentp->mode), uentp->from);
			}
		}
	}
	return j;
		...
		if ((*fptr) (farg, uentp)) {
			...
			if (uentp->invisible == 1) {
				if (HAS_PERM(PERM_SYSOP | PERM_SEECLOAK, currentuser)) {
					// \033[1;32m隐身中   \033[m
					prints("\033[1;32m\xD2\xFE\xC9\xED\xD6\xD0   \033[m");
					continue;
				} else
					continue;
			}
			...
			if (num == 1)
				// 目前在站上，状态如下：\n
				prints("\xC4\xBF\xC7\xB0\xD4\xDA\xD5\xBE\xC9\xCF\xA3\xAC\xD7\xB4\xCC\xAC\xC8\xE7\xCF\xC2\xA3\xBA\n");
...
show_small_bm(char *board)
{
	...
	for (i = 4; i < 10; i++) {	//仅对12个小版务的显示
		if (bptr->header.bm[i][0] == 0) {
			if (i == 4) {
				// 本版无小版主
				prints("\xB1\xBE\xB0\xE6\xCE\xDE\xD0\xA1\xB0\xE6\xD6\xF7 ");
				continue;
			} else {
				prints("             ");
				continue;
