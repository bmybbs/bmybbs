	...
	if (last_line == 0) {
		if (cmdmode == RMAIL) {
			...
			if (friendflag)
				...
				strcpy(desc, "坏人");
			sprintf(buf, "没有任何%s (A)新增%s (Q)离开？[Q] ", desc,
				desc);
		...
		if (talkrequest) {
			...
			if (lbc < 9 || ch == Ctrl('H') || ch == '\177') {
				...
				sprintf(buf, "[\033[36m%.12s\033[33m]",
					currentuser.userid);
				prints("\033[1;44;33m转到:[\033[36m%10s\033[33m]"
...
int SR_BMfunc(int ent, struct fileheader *fileinfo, char *direct) {
	...
	static const char *subBMitems[] = {"相同主题", "相同作者"}; //add by mintbaggio
	...
	move(t_lines - 3, 0);
	clrtoeol();
	sprintf(buf, "确定要执行%s[%s]吗", subBMitems[dotype-1], SR_BMitems[BMch - 1]);
	...
	if ((digestmode == 2 || digestmode == 3 || digestmode == 4 || digestmode == 5) && (BMch != 4 && BMch != 5))
		return FULLUPDATE;
	move(t_lines - 2, 0);
	sprintf(buf, "是否从%s第一篇开始%s (Y)第一篇 (N)目前这一篇", (dotype == 2) ? "该作者" : "此主题",
		SR_BMitems[BMch - 1]);
...
int sread(int passonly, int readfirst, int pnum, int auser, struct fileheader *ptitle) {
	...
	if (passonly == 0 || passonly == 4) {
		if (readfirst)
			...
			move(t_lines - 1, 0);
			clrtoeol();
			prints("\033[1;44;31m[%8s] \033[33m下一封 <Space>,<Enter>,↓,n│上一封 ↑,U,l                          \033[m", auser ? "相同作者" : "主题阅读");
			switch (egetch()) {
...
redo:
			if (ch != KEY_UP && ch != KEY_DOWN && (ch <= 0 || strchr("RrEexp", ch) == NULL)) {
				move(t_lines - 1, 0);
				clrtoeol();
				prints("\033[1;44;31m[%8s] \033[33m回信 R │ 结束 Q,← │下一封 ↓,n,Enter│上一封 ↑,l│ ^R 回给作者   \033[m", auser ? "相同作者" : "主题阅读");
				ch = egetch();
			}
			switch (ch) {
			case Ctrl('Y'):
