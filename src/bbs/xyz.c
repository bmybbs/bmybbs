...
static const char * const g_permstrings[] = {
	...
	"½ûÖ¹Ê¹ÓÃÇ©Ãûµµ",	/* PERM_DENYSIG */
	"ÒşÉíÊõ",		/* PERM_CLOAK */
	"¿´´©ÒşÉíÊõ",		/* PERM_SEECLOAK */
	"ÕÊºÅÓÀ¾Ã±£Áô",		/* PERM_XEMPT */
	...
	"Ç¿ÖÆºô½Ğ",		/* PERM_FORCEPAGE */
	"ÑÓ³¤·¢´ôÊ±¼ä",		/* PERM_EXT_IDLE */
	"´óĞÅÏä",		/* PERM_SPECIAL1 */
	"ÌØÊâÈ¨ÏŞ 2",		/* PERM_SPECIAL2 */
	"ÌØÊâÈ¨ÏŞ 3",		/* PERM_SPECIAL3 */
	"Çø³¤",			/* PERM_SPECIAL4 */
	"±¾Õ¾¼à²ì×é",		/* PERM_SPECIAL5 */
	"±¾Õ¾Á¢·¨»á",		/* PERM_SPECIAL6 */
	"ÌØÊâÈ¨ÏŞ 7",		/* PERM_SPECIAL7 */
	"¸öÈËÎÄ¼¯",		/* PERM_SPECIAL8 */
	"½ûÖ¹·¢ĞÅÈ¨",		/* PERM_DENYMAIL */
};

static const char *const g_user_definestr[NUMDEFINES] = {
	"ºô½ĞÆ÷¹Ø±ÕÊ±¿ÉÈÃºÃÓÑºô½Ğ",	/* DEF_FRIENDCALL */
	"½ÓÊÜËùÓĞÈËµÄÑ¶Ï¢",	/* DEF_ALLMSG */
	"½ÓÊÜºÃÓÑµÄÑ¶Ï¢",	/* DEF_FRIENDMSG */
	"ÊÕµ½Ñ¶Ï¢·¢³öÉùÒô",	/* DEF_SOUNDMSG */
	"Ê¹ÓÃ²ÊÉ«",		/* DEF_COLOR */
	"ÏÔÊ¾»î¶¯¿´°æ",		/* DEF_ACBOARD */
	"ÏÔÊ¾Ñ¡µ¥µÄÑ¶Ï¢À¸",	/* DEF_ENDLINE */
	"±à¼­Ê±ÏÔÊ¾×´Ì¬À¸",	/* DEF_EDITMSG */
	"Ñ¶Ï¢À¸²ÉÓÃÒ»°ã/¾«¼òÄ£Ê½",	/* DEF_NOTMSGFRIEND */
	"Ñ¡µ¥²ÉÓÃÒ»°ã/¾«¼òÄ£Ê½",	/* DEF_NORMALSCR */
	"·ÖÀàÌÖÂÛÇøÒÔ New ÏÔÊ¾",	/* DEF_NEWPOST */
	"ÔÄ¶ÁÎÄÕÂÊÇ·ñÊ¹ÓÃÈÆ¾íÑ¡Ôñ",	/* DEF_CIRCLE */
	"ÔÄ¶ÁÎÄÕÂÓÎ±êÍ£ÓÚµÚÒ»ÆªÎ´¶Á",	/* DEF_FIRSTNEW */
	"½øÕ¾Ê±ÏÔÊ¾ºÃÓÑÃûµ¥",	/* DEF_LOGFRIEND */
	"½øÕ¾Ê±ÏÔÊ¾±¸ÍüÂ¼",	/* DEF_INNOTE */
	"ÀëÕ¾Ê±ÏÔÊ¾±¸ÍüÂ¼",	/* DEF_OUTNOTE */
	"ÀëÕ¾Ê±Ñ¯ÎÊ¼Ä»ØËùÓĞÑ¶Ï¢",	/* DEF_MAILMSG */
	"Ê¹ÓÃ×Ô¼ºµÄÀëÕ¾»­Ãæ",	/* DEF_LOGOUT */
	"ÎÒÊÇÕâ¸ö×éÖ¯µÄ³ÉÔ±",	/* DEF_SEEWELC1 */
	"ºÃÓÑÉÏÕ¾Í¨Öª",		/* DEF_LOGINFROM */
	"¹Û¿´ÁôÑÔ°æ",		/* DEF_NOTEPAD */
	"²»ÒªËÍ³öÉÏÕ¾Í¨Öª¸øºÃÓÑ",	/* DEF_NOLOGINSEND */
	"Ö÷ÌâÊ½¿´°æ",		/* DEF_THESIS */
	"ÊÕµ½Ñ¶Ï¢µÈºò»ØÓ¦»òÇå³ı",	/* DEF_MSGGETKEY */
	"ºº×ÖÕû×Ö´¦Àí",		/* DEF_DELDBLCHAR */
	"Ê¹ÓÃGBÂëÔÄ¶Á",		/* DEF_USEGB KCN 99.09.03 */
	"Ê¹ÓÃ¶¯Ì¬µ×Ïß",		/* DEF_ANIENDLINE */
	"³õ´Î·ÃÎÊ°æÃæÌáÊ¾½øÈë¾«»ªÇø",	/* DEF_INTOANN */
	"·¢±íÎÄÕÂÊ±ÔİÊ±ÆÁ±ÎMSG",	/* DEF_POSTNOMSG */
	"½øÕ¾Ê±¹Û¿´Í³¼ÆĞÅÏ¢",	/* DEF_SEESTATINLOG */
	"¹ıÂË¿ÉÄÜÁîÈË·´¸ĞĞÅÏ¢",	/* DEF_FILTERXXX */
	//"ÊÕÈ¡Õ¾ÍâĞÅ¼ş",		/* DEF_INTERNETMAIL */
	"½øÕ¾Ê±¹Û¿´È«¹úÊ®´óÅÅĞĞ°ñ"	/* DEF_NEWSTOP10 */
};
