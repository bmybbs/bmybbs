...
//			prints("²»ÕıÈ·µÄÖĞÓ¢ÎÄÕÊºÅ\n");
		if (id_with_num(newuser.userid)) {
			...
			prints
			    ("±§Ç¸, Äú²»ÄÜÊ¹ÓÃÕâ¸ö×Ö×÷ÎªÕÊºÅ¡£ ÇëÏë¹ıÁíÍâÒ»¸ö¡£\n");
	...
	if (urec->lastlogin - urec->firstlogin < 3 * 86400) {
		if (urec->numlogins == 1) {
			...
			/*
			   ...
			   getdata(3,0,"ÇëÊäÈëÄãµÄĞÔ±ğ: [1]ÄĞµÄ [2]Å®µÄ [3]²»¸æËßÄã (1-3): ",
			   ans,2,DOECHO,EA);
			   }
			   switch (ans[0]) {
			   case '1': strcpy (buf2, "Ğ¡µÜ"); break;
			   case '2': strcpy (buf2, "Ğ¡Å®×Ó"); break;
			   case '3':
			   default : strcpy (buf2, "ÎÒ"); break;
			   } */
			strcpy(buf, "ÎÒ");
			...
			if ((fout = fopen(buf, "w")) != NULL) {
				...
				fprintf(fout, "ÎÒÊÇ %s (%s), À´×Ô %s\n",
					...
					fromhost);
				fprintf(fout,
					"½ñÌì%s³õÀ´´ËÕ¾±¨µ½, Çë´ó¼Ò¶à¶àÖ¸½Ì¡£\n",
					buf2);
				move(5, 0);
				prints
				    ("Çë×÷¸ö¼ò¶ÌµÄ¸öÈË¼ò½é, Ïò±¾Õ¾ÆäËûÊ¹ÓÃÕß´ò¸öÕĞºô\n");
				prints
				    ("(×î¶àÈıĞĞ, Ğ´Íê¿ÉÖ±½Ó°´ <Enter> ÌøÀë)....");
