...
char *pmt, *abort;
{
	...
	if (uinfo.mode == POSTING) {
		...
		if (lines < total * 0.35 - MAXSIGLINES) {
			move(y, 0);
			prints
			    ("×¢Òâ£º±¾ÆªÎÄÕÂµÄÒıÑÔ¹ı³¤, ½¨ÒéÄúÉ¾µôÒ»Ğ©²»±ØÒªµÄÒıÑÔ.\n");
			y += 3;
		}
		if (len < 40 || lines < 1) {
			move(y, 0);
		...
		if (w) {
			strcpy(pmt,
			       "(E)ÔÙ±à¼­, (F)²»»»ĞĞ·¢³ö£¬(S)·¢³ö, (A)È¡Ïû or (T)¸ü¸Ä±êÌâ? [E]: ");
		}
	}
...
int modifyheader;
{
	...
	if (uinfo.mode != CCUGOPHER) {
		if (uinfo.mode == POSTING) {
			if (local_article == 1)
				strcpy(p_buf,
				       "(L)²»×ªĞÅ, (S)·¢±í, (F)×Ô¶¯»»ĞĞ·¢³ö, (A)È¡Ïû, (T)¸ü¸Ä±êÌâ or (E)ÔÙ±à¼­? [L]: ");
			else {
				if (local_article == -1)
					strcpy(p_buf,
					       "(L)²»×ªĞÅ, (F)×Ô¶¯»»ĞĞ·¢³ö, (A)È¡Ïû, (T)¸ü¸Ä±êÌâ or (E)ÔÙ±à¼­? [L]: ");
				else
					strcpy(p_buf,
					       "(S)·¢±í, (L)²»×ªĞÅ£¬(F)×Ô¶¯»»ĞĞ·¢³ö, (A)È¡Ïû, (T)¸ü¸Ä±êÌâ or (E)ÔÙ±à¼­? [S]: ");
			}
		} else if (uinfo.mode == SMAIL)
			strcpy(p_buf,
			       "(S)¼Ä³ö, (F)×Ô¶¯»»ĞĞ·¢³ö£¬(A)È¡Ïû, or (E)ÔÙ±à¼­? [S]: ");
		else
			strcpy(p_buf,
			       "(S)´¢´æµµ°¸, (F)×Ô¶¯»»ĞĞ·¢³ö£¬(A)·ÅÆú±à¼­, (E)¼ÌĞø±à¼­? [S]: ");
