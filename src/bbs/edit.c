...
msgline(int ck)
{
	...
	sprintf(buf2,
		"×´Ì¬ [[32m%s[33m][[32m%4.4d[33m,[32m%3.3d[33m][[32m%s[33m] Ê±¼ä",
...
input_tools()
{
	char msg[] = { "1.¼Ó¼õ³Ë³ı  2.Ò»¶şÈıËÄ  3.£Á£Â£Ã£Ä  4.ºáÊúĞ±»¡ £¿[Q]" };
	char *ansi1[7][10] = {
		{"£«", "£­", "¡Á", "¡Â", "¡À", "¡ß", "¡à", "¡Ê", "¡Ô", "¡Ø"},
		{"¡Æ", "¡Ç", "¡È", "¡É", "¡Ò", "¡Ó", "¡Ã", "¡Ä", "¡Å", "¡Ë"},
		{"¡Õ", "¡Ö", "¡×", "¡Ù", "¡Ú", "¡Û", "¡Ü", "¡İ", "¡Ş", "¡Ï"},
	...
	if (enabledbchar && (currline->data[currpnt] & 0x80)) {
		for (i = 0; i < currpnt; i++)
			if (dbchar)
				...
				dbchar = 1;
		/* ÕâÀïÊÇÎâÈçºêÓÚ1999ÄêÔªÔÂ8ÈÕĞŞ¸Ä¹ı */
		if (currpnt < currline->len - 1) {
			if (currline->data[currpnt + 1] & 0x80) {
				/*  ÕâÑù¿ÉÒÔÈ·±£É¾³ıµÄÊÇÒ»¸öÕû×Ö */
				if (dbchar) {
...
int modifyheader;
{
	...
	/* Added by deardragon 1999.11.13  ½ûÖ¹ĞŞ¸ÄÎÄÕÂÍ·²¿ĞÅÏ¢ */
	/* Èç¹ûÊÇĞŞ¸ÄÎÄÕÂ, ÏÈ°ÑÎÄÕÂÍ·²¿ĞÅÏ¢±£´æÆğÀ´, ²¢Ê¹Ö¸ÕëÖ¸ÏÂ±à¼­Çø */
...
int mode;
{
	...
	//ÓĞÊ±ºò¾Í²»»á´æÔÚ°¡,±ÈÈçÎÒÉ¾³ıÁË°æÃæ µ«ÊÇÄ¿Â¼»¹ÔÚ...
	if (bp)
	...
	fprintf(fp, "±ê  Ìâ: %s\n", save_title);
	fprintf(fp, "·¢ĞÅÕ¾: %s (%24.24s), %s", MY_BBS_NAME, ctime(&now),
		local_article ? "±¾Õ¾(" MY_BBS_DOMAIN ")" : "×ªĞÅ("
		MY_BBS_DOMAIN ")");
	if (in_mail)
		fprintf(fp, "\nÀ´  Ô´: %s", fromhost);
	fprintf(fp, "\n\n");
	...
	if (uinfo.mode == POSTING) {
		...
		while (p != NULL) {
			if (!sig) {
				...
				if (strcmp(p->data, "--") == 0)
					sig = 1;
				else if (ch != ':' && ch != '>' && ch != '=' && !strstr(p->data, "µÄ´ó×÷ÖĞÌáµ½: ¡¿")) {
					lines++;
					len += strlen(p->data);
				}
			}
		...
		if (len < 40 || lines < 1) {
			move(y, 0);
			prints("×¢Òâ£º±¾ÆªÎÄÕÂ¹ıÓÚ¼ò¶Ì, ÏµÍ³ÈÏÎªÊÇ¹àË®ÎÄÕÂ.\n");
			y += 3;
		}
		if (w) {
			strcpy(pmt, "(E)ÔÙ±à¼­, (F)²»»»ĞĞ·¢³ö£¬(S)·¢³ö, (A)È¡Ïû or (T)¸ü¸Ä±êÌâ? [E]: ");
		}
	}
...
int modifyheader;
{
	...
	//local_article: -1 ²»×ªĞÅ°æÃæ  0 ×ªĞÅ°æÃæÄ¬ÈÏ²»×ªĞÅ  1 ×ªĞÅ
	if (uinfo.mode != CCUGOPHER) {
		if (uinfo.mode == POSTING) {
			if (local_article == 1)
				strcpy(p_buf, "(L)²»×ªĞÅ, (S)·¢±í, (F)×Ô¶¯»»ĞĞ·¢³ö, (A)È¡Ïû, (T)¸ü¸Ä±êÌâ or (E)ÔÙ±à¼­? [L]: ");
			else {
				if (local_article == -1)
					strcpy(p_buf, "(L)²»×ªĞÅ, (F)×Ô¶¯»»ĞĞ·¢³ö, (A)È¡Ïû, (T)¸ü¸Ä±êÌâ or (E)ÔÙ±à¼­? [L]: ");
				else
					strcpy(p_buf, "(S)·¢±í, (L)²»×ªĞÅ£¬(F)×Ô¶¯»»ĞĞ·¢³ö, (A)È¡Ïû, (T)¸ü¸Ä±êÌâ or (E)ÔÙ±à¼­? [S]: ");
			}
		} else if (uinfo.mode == SMAIL)
			strcpy(p_buf, "(S)¼Ä³ö, (F)×Ô¶¯»»ĞĞ·¢³ö£¬(A)È¡Ïû, or (E)ÔÙ±à¼­? [S]: ");
		else
			strcpy(p_buf, "(S)´¢´æµµ°¸, (F)×Ô¶¯»»ĞĞ·¢³ö£¬(A)·ÅÆú±à¼­, (E)¼ÌĞø±à¼­? [S]: ");
	...
	if (!aborted) {
		...
		if (!strncmp(filename, "2nd/", 4)) {
			...
			write_header(fp, 0);
		/* Added by deardragon 1999.11.13 ½ûÖ¹ĞŞ¸ÄÎÄÕÂÍ·²¿ĞÅÏ¢ */
		/* Èç¹ûÊÇĞŞ¸ÄÎÄÕÂ, ÔòÔÚ±£´æÊ±, »Ö¸´ÎÄÕÂÍ·ĞÅÏ¢. */
		/* Patched by lepton 2002.11.23 */
...
#define SAVE_PART_TO    "°Ñ±ê¼Ç¿éĞ´Èë¼ôÌù²¾µÚ¼¸Ò³? (0-7) [Ô¤ÉèÎª 0]"
...
#define CHOOSE_BIG5     "ÄÚÂë±í? 0)±êµã 1)·ûºÅ 2)±í¸ñ 3)×¢Òô 4)Æ½¼Ù 5)Æ¬¼Ù 6)Ï£À° 7)Êı×Ö"
...
int modifyheader;
{
	...
	if (file_size(filename) > 0) {
		...
		if (retv)
			if (askyn("±¾ÎÄ°üº¬¸½¼ş£¬±à¼­ºó½«¶ªÊ§¸½¼ş£¬È·¶¨Òª±à¼­Âğ£¿", NA, NA) == NA)
				return -1;
	}*/
	t = showansi;
	showansi = 0;
