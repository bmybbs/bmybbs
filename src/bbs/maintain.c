...
int m_info(const char *s) {
	...
	move(1, 0);
	// 请输入使用者代号:
	usercomplete("\xC7\xEB\xCA\xE4\xC8\xEB\xCA\xB9\xD3\xC3\xD5\xDF\xB4\xFA\xBA\xC5: ", genbuf, sizeof genbuf);
	if (*genbuf == '\0') {
		clear();
		return -1;
	}
...
int m_editbrd(const char *s) {
	...
	make_blist_full();
	// 输入讨论区名称:
	namecomplete("\xCA\xE4\xC8\xEB\xCC\xD6\xC2\xDB\xC7\xF8\xC3\xFB\xB3\xC6: ", bname, sizeof bname);
	if (*bname == '\0') {
		move(2, 0);
		// 错误的讨论区名称
...
int m_register(const char *s) {
	...
	if (ans[0] == '1') {
		...
		move(1, 0);
		// 请输入要查询的代号:
		usercomplete("\xC7\xEB\xCA\xE4\xC8\xEB\xD2\xAA\xB2\xE9\xD1\xAF\xB5\xC4\xB4\xFA\xBA\xC5: ", uident, sizeof uident);
		if (uident[0] != '\0') {
			if (!getuser(uident)) {
				move(2, 0);
				// 错误的使用者代号...
...
do_ordainBM(const char *userid, const char *abname)
{
	...
	if (userid)
		...
		// 输入欲任命的使用者帐号:
		usercomplete("\xCA\xE4\xC8\xEB\xD3\xFB\xC8\xCE\xC3\xFC\xB5\xC4\xCA\xB9\xD3\xC3\xD5\xDF\xD5\xCA\xBA\xC5: ", genbuf, sizeof genbuf);
	if (genbuf[0] == '\0') {
		clear();
		return 0;
	}
	...
	if (abname)
		...
		make_blist_full();
		// 输入该使用者将管理的讨论区名称:
		namecomplete("\xCA\xE4\xC8\xEB\xB8\xC3\xCA\xB9\xD3\xC3\xD5\xDF\xBD\xAB\xB9\xDC\xC0\xED\xB5\xC4\xCC\xD6\xC2\xDB\xC7\xF8\xC3\xFB\xB3\xC6: ", bname, sizeof bname);
	}
	if (*bname == '\0') {
		move(5, 0);
		// 错误的讨论区名称
...
do_retireBM(const char *userid, const char *abname)
{
	...
	if (userid)
		...
		// 输入欲离任的使用者帐号:
		usercomplete("\xCA\xE4\xC8\xEB\xD3\xFB\xC0\xEB\xC8\xCE\xB5\xC4\xCA\xB9\xD3\xC3\xD5\xDF\xD5\xCA\xBA\xC5: ", genbuf, sizeof genbuf);
	if (genbuf[0] == '\0') {
		clear();
		return 0;
	}
