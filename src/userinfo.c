...
char *addr, char *phone, char *assoc, char *email)
{
	...
	memcpy(&uinfo, &lookupuser, sizeof (uinfo));
			...
			mail_file("etc/s_fill", uinfo.userid,
				  "¹§ìûÄúÍ¨¹ıÉí·İÑéÖ¤"); // Õâ¸öµØ·½ÓĞ¸öè¦´Ã£¬¾ÍÊÇ·¢ĞÅÈËÎª±¾ÈË
...
//           prints("Ident ×ÊÁÏ   : %s\n", u->ident );
	...
	prints("ÉÏ´ÎÀëÕ¾Ê±¼ä : %s",
	       u->lastlogout ? ctime(&u->lastlogout) : "²»Ïê\n");
	prints("ÉÏÕ¾´ÎÊı     : %d ´Î\n", u->numlogins);
	if (real) {
		prints("ÎÄÕÂÊıÄ¿     : %d\n", u->numposts);
	}
				...
				if (sysconf_str("EMAILFILE") != NULL) {
					...
					prints
					    ("\nÄúËùÌîµÄµç×ÓÓÊ¼şµØÖ· ¡¾[1;33m%s[m¡¿\n",
					     newinfo.email);
					prints
					    ("²¢·ÇºÏ·¨Ö® UNIX ÕÊºÅ£¬ÏµÍ³²»»áÍ¶µİ×¢²áĞÅ£¬Çë°ÑËüĞŞÕıºÃ...\n");
					pressanykey();
				}
			}
		}
...
*/
	...
	*/
	...
	int result;
	getdata(13, 0, "ĞÅÏäÓÃ»§Ãû(ÊäÈëx·ÅÆúÑéÖ¤£¬ĞÂÉú×¢²áÇëÊäÈëÓÃ»§Ãûtest£¬ÃÜÂëtest) >>  ", user, USER_LEN, DOECHO, YEA);
