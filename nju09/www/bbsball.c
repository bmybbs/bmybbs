...
{	//modify by mintbaggio 20040829 for new www
	...
	printf("<table border=1><tr><td>序号</td><td>黑名单代号</td><td>黑名单说明</td><td>从黑名单删除</td></tr>");
	for (i = 0; i < badnum; i++) {
		printf("<tr><td>%d</td>", i + 1);
		printf("<td><a href=bbsqry?userid=%s>%s</a></td>", bbb[i].id, bbb[i].id);
		printf("<td>%s</td>\n", nohtml(bbb[i].exp));
		printf("<td>[<a onclick='return confirm(\"确实删除吗?\")' href=bbsbdel?userid=%s>删除</a>]</td></tr>", bbb[i].id);
	}
	printf("</table><hr>\n");
