...
bbsmdoc_main()
{
	...
	for (i = 0; i < 20; i++) {
		...
		printf("<td><input style='height:18px' name=box%s type=checkbox>", fh2fname(&x));
		printf("<td>%s<td>%s", flag_str_bm(x.accessed), userid_str(fh2owner(&x)));
	...
	printf("<input type=button value=加删除标记 onclick="
	...
	printf("</form>\n");
	if (start > 0) {
		printf("<a href=bbsmdoc?board=%s&start=%d>上一页</a> ", board, start < 20 ? 0 : start - 20);
	}
	if (start < total - 20) {
		printf("<a href=bbsmdoc?board=%s&start=%d>下一页</a> ", board, start + 20);
	}
