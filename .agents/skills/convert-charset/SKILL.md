...
- 例如原字符串 `"中文xyz"` 应改写成 `"\xD6\xD0\xCE\xC4xyz"`。
- 例如原字符串 `"中文abc"` 也就是当字符和 `[0-9A-Fa-f]` 连接在一起时，应改写成 `("\xD6\xD0\xCE\xC4" "abc")` 避免 hex escape out of range 错误。
- 例如原字符串 `"├xyz"`，如果该符号在原文件中的字节是 `A9 C0`，则应改写为 `"\xA9\xC0xyz"`。
- 某些使用 GBK 编码（可能极少数情况存在 BIG5 编码）的字符的第二字节，其数值可能刚好位于 ASCII 可打印的字符集中。这种情况下应使用 `\xHH` 形式表示，因为该字节连同前一字节组合在一起才表示一个字符。如果使用 ASCII 表示，虽然运行时结果正确，但是在阅读源码时会产生歧义。
