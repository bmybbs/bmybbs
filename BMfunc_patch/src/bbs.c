...
static int g_board_names(struct boardmem *fhdrp);
...
show_cake(char *filename, int num)
{
	...
	while (NULL != fgets(line, 200, fp)) {
		...
		if (line[0] == '#' && line[1] != '#') {
			...
			if (i == 0) {
				sprintf(line, " 请输入数字'%d': ", num0 % 10);
				getdata(20, 1, line, str, 29, DOECHO, YEA);
				if (atoi(str) != num0 % 10) {
					prints
					    ("\033[22;1H�..好象不大对吧, 您还有一次机会...");
...
char *direct;
{
	...
	clear();
	prints
	    ("[1m请注意：本站站规规定：内容相同或类似的文章严禁在[31m3(不含)[37m个以上讨论区重复张贴。\n");
	prints
	    ("[1m转贴超过3个讨论区者除所贴文章会被全部删除之外，还将被剥夺全站发表文章的权利。\n");
	prints
	    ("[1m             请大家共同维护 BBS 的环境，节省系统资源。谢谢合作。\n[0m");
	move(4, 0);
...
{                                                                                	if (!IScurrBM) return DONOTHING;
			...
			if (get_num_records(digestdir, sizeof (digest)) > 4) {
			move(3, 0);
			clrtobot();
			move(4, 10);
			prints ("抱歉，置底数量超过3篇，无法再加入...\n");
			pressanykey();
			return PARTUPDATE; 
			    }
     			}
...
char *direct;
{
	...
	if (haspostperm(currboard)
		...
		prints
		    ("[1;44;31m[阅读文章] [33m回文 R│推荐给网友 E│结束 ←│上一封↑l│下一封↓n│主题阅读 x p[m");
	} else {
		prints
		    ("[1;44;31m[阅读文章] [33m结束 Q,←│上一封 ↑,l│下一封 n, <Space>,<Enter>,↓│主题阅读 x p [m");
	}
...
int mode;
{
	...
	if (mode == 0) {
		...
		if (in_mail == YEA)
			fprintf(of,
				"[1;37m【 以下文字转载自 [32m%s [37m的信箱 】\n",
				currentuser.userid);
		else
			fprintf(of,
				"[1;37m【 以下文字转载自 [32m%s [37m讨论区 】\n",
				quote_board);
...
char *direct;
{
	...
	if (!in_mail && !hideboard(currboard)) {
		...
		case -1:
			...
			return FULLUPDATE;
		case -2:
			snprintf(mtitle, sizeof (mtitle), "[修改报警] %s %.60s",
...
char *direct;
{
	...
	if (askyn("确定删除", NA, NA) == YEA) {
		if ((ret = delete_range(direct, inum1, inum2)) < 0) {
			...
			prints("无法删除:%d\n", ret);
			getdata(8, 0,
				"区段删除错误,如果想修复,请确定[35m无人在本版执行区段删除操作并按'Y'[0m (Y/N)? [N]: ",
				num1, 10, DOECHO, YEA);
			if (*num1 == 'Y' || *num1 == 'y') {
...
notepad()
{
	...
	if ((in = fopen(tmpname, "w")) != NULL) {
		...
		if (note1[0] != 'N' && note1[0] != 'n') {
			...
			fprintf(in,
				"[1;34m□[44m□□□□□□□□□□□□□□□□□[36m酸[32m甜[33m苦[31m辣[37m版[34m□□□□□□□□□□□□□□□[40m□[m\n");
			fprintf(in,
				"[1;34m□[32;44m %-48s[32m在 [36m%.19s[32m 离开时留下的话  [m\n",
				tmp, Ctime(thetime));
			for (n = 0; n < i; n++) {
				if (note[n][0] == '\0')
					break;
...
Goodbye()
{
	...
	*quote_file = '\0';
	...
	if (strcmp(currentuser.userid, "guest") != 0) {
		if (USE_NOTEPAD == 1)
			prints
			    ("[[1;33m3[m] 写写[1;32m留[33m言[35m版[m罗\n");
	}
	prints("[[1;33m4[m] 不寄罗，要离开啦\n");
	sprintf(spbuf, "你的选择是 [[1;32m4[m]：");
...
marked_mode()
{
	...
	if (digestmode == 3 || digestmode == YEA) {
		...
		ans[0] = '\0';
		//getdata(t_lines - 1, 0,
		//      "请选择模式:0)取消 1)防水模式 2)m文章 3)相关标题 4)同作者 5)垃圾桶 6)未读[0]:",
		//      ans, 2, DOECHO, NA);
		getdata(t_lines - 1, 0,
...
b_notes_edit()
{
	...
	while (1) {
		getdata(3, 0,
			"编辑或删除本讨论区的 (0)离开 (1)一般备忘录 (2)秘密备忘录? [1] ",
			ans, 2, DOECHO, YEA);
		if (ans[0] == '0')
			return FULLUPDATE;
		if (ans[0] == '\0')
	...
	sprintf(buf2, "(E)编辑 (D)删除 %4s备忘录? [E]: ",
		(notetype == 1) ? "一般" : "秘密");
	getdata(5, 0, buf2, ans, 2, DOECHO, YEA);
	if (ans[0] == 'D' || ans[0] == 'd') {
		move(6, 0);
		sprintf(buf2, "真的要删除%4s备忘录",
			(notetype == 1) ? "一般" : "秘密");
		if (askyn(buf2, NA, NA)) {
			move(7, 0);
			prints("备忘录已经删除...\n");
